...
class BracketedImagesAligner:
    ...
    def _match_and_filter(
        ...
        if len(mkpts1) < 10:
            raise ValueError("Too few correspondences found!")
    ...
    def _warp_local_tps(
        self, img_ref, target_img_path, pts_ref, pts_target
    ) -> np.ndarray:
    ...
    def align(
        ...
        for normalized_image_path, original_image_path in zip(
            ...
            original_image_warped_global = cv2.warpPerspective(
                original_image, H, (w_ref, h_ref)
            )
            normalized_image_warped_global = cv2.warpPerspective(
                normalized_image, H, (w_ref, h_ref)
            )
    ...
    def create_difference_image(
        ...
        diff_amplified = np.clip(
            diff.astype(np.float32) * amplification, 0, 255
        ).astype(np.uint8)
...
if __name__ == "__main__":
    if len(sys.argv) != 7:
        print(
            "Usage: python aligner.py ref_image.jpg normalized_over.jpg normalized_under.jpg original_over.jpg original_under.jpg output_folder"
        )
        print(
            "  The ref image is the middle exposure one and is used as reference for the alignment."
        )
        print(
            "  The normalized images are the over and under exposed images after exposure normalization."
        )
        print(
            "  The original images are the over and under exposed images before exposure normalization."
        )
        print(
            "  The output folder is where the aligned images and diagnostics will be saved."
        )
        sys.exit(1)
    ...
    aligned_normalized, aligned_original = aligner.align(
        ref_image_path, normalized_images_paths, original_images_paths, output_folder
    )

    # Diagnostics image generation
    out = Path(output_folder)
    for i in [1, 2]:
        checker_normalized = BracketedImagesAligner.create_checkerboard_comparison(
            aligned_normalized[0], aligned_normalized[i]
        )
        cv2.imwrite(str(out / f"checker_normalized_ref_vs_{i}.jpg"), checker_normalized)
        checker_original = BracketedImagesAligner.create_checkerboard_comparison(
            aligned_original[0], aligned_original[i]
        )
        cv2.imwrite(str(out / f"checker_original_ref_vs_{i}.jpg"), checker_original)

        diff_normalized = BracketedImagesAligner.create_difference_image(
            aligned_normalized[0], aligned_normalized[i]
        )
        cv2.imwrite(str(out / f"diff_normalized_ref_vs_{i}.jpg"), diff_normalized)
        diff_original = BracketedImagesAligner.create_difference_image(
            aligned_original[0], aligned_original[i]
        )
