...
class HdrResultMerger:
    ...
    def merge_hdr_results(
        self, hdr_with_ghost: np.ndarray, hdr_without_ghost: np.ndarray, soft_mask: np.ndarray
    ) -> np.ndarray:
...
if __name__ == "__main__":
    if len(sys.argv) != 5:
        print("Usage: python hdr_merger.py hdr_with_ghost.jpg hdr_without_ghost.jpg soft_mask.jpg output_path")
        print("  hdr_with_ghost and hdr_without_ghost are the two HDR images to be merged.")
    ...
    soft_mask = cv2.imread(soft_mask_path, cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255.0
